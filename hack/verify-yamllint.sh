...
yamllint -f parsable charts/latest/blobfuse-csi-driver/templates/*.yaml | grep -v "line too long" | grep -v "too many spaces inside braces" | grep -v "missing document start" | grep -v "syntax error" > /tmp/yamllint.log
